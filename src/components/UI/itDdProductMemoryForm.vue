<template>
  <div @click="isVisible = !isVisible" id="dd_input" class="ui floating dropdown labeled icon button" >
    ...
    <span v-else class="text">Выберите форм фактор памяти
      <svg v-if="!isVisible" class="arrow"  style="width: 24px; height: 24px;position: absolute;right: 5px;top: 5px;">
          <path fill="currentColor" d="M7,10L12,15L17,10H7Z" />
      </svg>
      <svg v-else class="arrow" style="width: 24px; height: 24px;position: absolute;right: 5px;top: 4px;">
    ...
    <div class="menu" v-if="isVisible" style="display: block">
      ...
      <div class="scrolling menu" style="display: block">
        <div @click="selectItem(form)" class="item" v-for="(form, index) in filteredProductMemoryForm" :key="`company-${index}`">
          {{ form.title }}
        </div>
        <div v-if="filteredProductMemoryForm.length === 0" class="item">
...
export default {
  ...
  computed: {
    filteredProductMemoryForm() {
      const query = this.searchQuery.toLowerCase();
      if (this.searchQuery === "") {
        return this.memory_form
      }
      return this.memory_form.filter((form) => {
        return Object.values(form).some((word) =>
  ...
  methods: {
    ...
    selectItem(form) {
      ...
      this.$emit('memory_form_id', this.selectedItem.id)
    },
    checkItem(){
      const result = this.memory_form.find((el) => el.id === parseInt(this.currentMemoryForm))
