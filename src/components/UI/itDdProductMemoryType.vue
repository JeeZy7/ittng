<template>
  <div @click="isVisible = !isVisible" id="dd_input" class="ui floating dropdown labeled icon button">
    ...
    <span v-else class="text">Выберите частоту памяти
      <svg v-if="!isVisible" class="arrow"  style="width: 24px; height: 24px;position: absolute;right: 5px;top: 5px;">
          <path fill="currentColor" d="M7,10L12,15L17,10H7Z" />
      </svg>
      <svg v-else class="arrow" style="width: 24px; height: 24px;position: absolute;right: 5px;top: 4px;">
    ...
    <div class="menu" v-if="isVisible" style="display: block">
      ...
      <div class="scrolling menu" style="display: block">
        <div @click="selectItem(type)" class="item" v-for="(type, index) in filteredProductMemoryType" :key="`company-${index}`">
          {{ type.title }}
        </div>
        <div v-if="filteredProductMemoryType.length === 0" class="item">
...
export default {
  ...
  computed: {
    filteredProductMemoryType() {
      const query = this.searchQuery.toLowerCase();
      if (this.searchQuery === "") {
        return this.memory_types
      }
      return this.memory_types.filter((type) => {
  ...
  methods: {
    ...
    selectItem(type) {
      ...
      this.$emit('memory_type_id', this.selectedItem.id)
    },
    checkItem(){
      const result = this.memory_types.find((el) => el.id === parseInt(this.currentMemoryType))
