<template>
  <div @click="isVisible = !isVisible" id="dd_input" class="ui floating dropdown labeled icon button">
    ...
    <span v-else class="text">Выберите вид памяти
     <svg v-if="!isVisible" class="arrow"  style="width: 24px; height: 24px;position: absolute;right: 5px;top: 5px;">
          <path fill="currentColor" d="M7,10L12,15L17,10H7Z" />
      </svg>
      <svg v-else class="arrow" style="width: 24px; height: 24px;position: absolute;right: 5px;top: 4px;">
    ...
    <div class="menu" v-if="isVisible" style="display: block">
      ...
      <div class="scrolling menu" style="display: block">
        <div @click="selectItem(variation)" class="item" v-for="(variation, index) in filteredProductMemoryVariation" :key="`company-${index}`">
          {{ variation.title }}
        </div>
        <div v-if="filteredProductMemoryVariation.length === 0" class="item">
...
<script>
export default {
  name: "itDdProductMemoryVariation",
  data(){
    return {
      searchQuery: "",
      selectedItem: null,
  ...
  props: {
    currentMemoryVariation: {
      type: String,
      required: true
    }
  },
  computed: {
    filteredProductMemoryVariation() {
      const query = this.searchQuery.toLowerCase();
      if (this.searchQuery === "") {
        return this.memory_variation
      }
      return this.memory_variation.filter((variation) => {
        return Object.values(variation).some((word) =>
  ...
  methods: {
    ...
    selectItem(variation) {
      ...
      this.$emit('memory_variation_id', this.selectedItem.id)
    },
    checkItem(){
      const result = this.memory_variation.find((el) => el.id === parseInt(this.currentMemoryVariation))
