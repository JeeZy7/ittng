...
<!--          <p>353535, Россия, Краснодарский край,<br> Темрюкский район, п. Волна,<br>ул. Таманская, д. 8</p>-->
        ...
        <div class="col-lg-3 pe-0">
          <h4 class="fw-bolder text-uppercase text-end mt-4 mb-2" style="font-size: 14px;">Отчет | Закупка товара</h4>
          <h6 class="text-end mb-5 pb-4">№ {{ report_index }}</h6>
<!--          <h6 class="mb-0 mt-3 text-end fw-normal mb-2"><span class="text-muted">Период отчета:</span> 2 квартал 2022</h6>-->
        </div>
      </div>
        ...
        <div class="table-responsive w-100">
          <table class="table table-bordered">
            ...
            <tr>
              ...
              <th class="text-end">C/З</th>
              <th class="text-end">Закупить</th>
            </tr>
            </thead>
            <tbody>
            <tr class="text-end" v-for="(item, index) in report_request" :key="item.id">
              <td class="text-start">{{ index+1 }}</td>
              <td class="text-start">{{ item.product_name }}</td>
              <td>{{ item.count }}</td>
              <td>{{ item.total }}</td>
              <td>{{ item.mean }}</td>
              <td style="background-color: #21ba2147">{{ item.order }}</td>
            </tr>
  ...
  <div class="container-fluid w-100" style="padding: 0rem 2rem 2rem; display: flex; justify-content: flex-end;margin-top: 1rem;" >
    <button @click="ExportToExcel" v-if="report_request.length" type="button" class="btn btn-outline-primary btn-icon-text me-2 mb-2 mb-md-0">
...
export default {
  ...
  methods: {
    ExportToExcel(){
      axios
          .post("http://ittng.ru/server/product_report_print.php", {
            report:this.report_request, organization:this.organization
          },{
            responseType: 'blob',
            headers: {
